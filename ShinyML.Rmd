            ...
            tabsetPanel(
                ...
                tabPanel("Machine learning simulator", # ------
                         ...
                         strong("C. The following sprout plot presents key summary statistics of each model.",
                                style = "color:#006666"),
                         fluidRow(
                             column(12, plotOutput("plt.summaryStats"))
                         )
                ),
...
server <- function(input, output) {
    ...
    ph  <- eventReactive(input$Shuffle, {
        ...
        if (input$SeedBirthday == T) {set.seed(11101991)}
        ...
        withProgress(message = "Modelling...", value = 0, {
            ...
            for(i in 1:length(progress)) {
                ...
                if (i == 4) {
                    ...
                    cf.counts.ElasticNet = func.regularizedLogistic(input.alpha = 0.5, ModelName = paste("EN") )[[1]]
                    cf.stats.ElasticNet = func.regularizedLogistic(input.alpha = 0.5, ModelName = paste("EN") )[[2]]
                    fitted.ElasticNet = func.regularizedLogistic(input.alpha = 0.5, ModelName = paste("EN") )[[3]]
                ...
                if (i == 8) {
                    ...
                    # Summarize all confusion matrix
                    df.confusionMatrix.all = cf.counts.LDA %>% rbind(cf.counts.QDA) %>%
                        ...
                        rbind(cf.counts.Bayes) %>% rbind(cf.counts.MostVoted) %>%
                        as_tibble()
                ...
                if (i == 9) {
                    ...
                    facet_wrap(~Model, nrow = 2) +
                        ...
                        theme(axis.text.x = element_text(angle = 90, vjust = .8, hjust = .8, size = 12, colour = "black"),
                    ...
                    # Summary statistics
                    plt.summaryStats = df.stats %>% ggplot(aes(x = Category, y = values, color = metrics)) +
    ...
    tab3.output = eventReactive(input$userfile, {
        ...
        return(list(d.fitted.all.named.1, h) )
    }) 
    ...
    tab3.heatmap = reactive({
        ...
        h = draw(plt.heatmap.batchPrediction, heatmap_legend_side = "bottom")
    })
